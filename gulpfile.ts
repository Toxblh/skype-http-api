...
const lib: LibTarget = {
  ...
  dist: {
    packageJsonMap: (old: buildTools.PackageJson): buildTools.PackageJson => {
      const version: string =
        options.devDist !== undefined ? `${old.version}-build.${options.devDist}` : old.version
  ...
  typedoc: {
    ...
    name: 'Skype Http',
    deploy: {
      repository: 'git@github.com:Toxblh/skype-http-api.git',
      branch: 'gh-pages',
    },
  },
  clean: {
...
gulp.task(
  'all:tsconfig.json',
  gulp.parallel('lib:tsconfig.json', 'test:tsconfig.json', 'example:tsconfig.json')
)
