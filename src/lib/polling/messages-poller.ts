  ...
  switch (native.resourceType) {
    ...
    case 'CustomUserProperties':
      resource = formatCustomUserPropertiesResource(
        native.resource as nativeResources.CustomUserPropertiesResource
      )
      break
    default:
      // tslint:disable-next-line:max-line-length
  ...
  protected async getMessages(): Promise<void> {
    try {
      ...
      if (res.statusCode !== 200) {
        const cause: UnexpectedHttpStatusError = UnexpectedHttpStatusError.create(
          res,
          new Set([200]),
          requestOptions
        )
  ...
  protected async getNotifications(): Promise<void> {
    try {
      const requestOptions: httpIo.GetOptions = {
        url: this.notificationUri
          ? this.notificationUri
          : await messagesUri.notifications(this.io, this.apiContext),
        cookies: this.apiContext.cookies,
        headers: {
          Authentication: 'skypetoken=' + this.apiContext.skypeToken.value,
        },
      ...
      if (res.headers['set-registrationtoken']) {
        ...
        // for debug, will remove
        console.log(
          `Updating registrationtoken -> getNotifications() -> ${res.headers['set-registrationtoken']}`
        )
      ...
      const body: {
        eventMessages?: nativeEvents.EventMessage[]
        next?: string
      } = JSON.parse(res.body)
      if (body.next) {
