...
export async function getLiveToken(options: GetLiveTokenOptions): Promise<string> {
  try {
    ...
    const err:
      | getLiveTokenErrors.GetLiveTokenError.Cause
      | WrongCredentialsError
      | WrongCredentialsLimitError = _err
    switch (err.name) {
      case httpErrors.RequestError.name:
...
export async function requestLiveToken(options: GetLiveTokenOptions): Promise<io.Response> {
  ...
  const ckTstCookie: toughCookie.Cookie = new (toughCookie.Cookie as any)({
    key: 'CkTst',
 ...
 */
export async function checkIfUpsellIsPresentAndDismiss(
  html: string,
  options: GetLiveTokenOptions
): Promise<string> {
...
export function scrapLiveToken(html: string): string {
  ...
  if (tokenValue === undefined || tokenValue === '') {
    if (
      ...
      html.indexOf(
        'sErrTxt:"You\\\'ve tried to sign in too many times with an incorrect account or password."'
      ) >= 0
    ) {
