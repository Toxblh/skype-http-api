...
export async function searchSkypeDirectory(
  io: io.HttpIo,
  apiContext: Context,
  userId: string
): Promise<String> {
  ...
  try {
    ...
    const sessionId: string = apiContext.registrationToken.endpointId.replace('{', '').replace('}', '')

    const url: string = `${uriBase}searchString=${encodeURIComponent(userId)}&requestId=${Math.round(
      new Date().getTime()
    ...
    if (res.statusCode !== 200) {
      return Promise.reject(new Incident('net', 'Bad response when searching skype directory'))
    }
