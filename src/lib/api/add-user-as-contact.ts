...
export async function addUserAsContact(io: io.HttpIo, apiContext: Context, userId: string): Promise<boolean> {
  const requestBody: RequestBody = {
    value: true,
  }

  const requestOptions: io.PutOptions = {
    url: `https://edge.skype.com/pcs-df/contacts/v2/users/${encodeURIComponent(
      apiContext.username
...
export async function removeUserFromContacts(
  io: io.HttpIo,
  apiContext: Context,
  userId: string
): Promise<boolean> {
